...
export default function AuthModal({
  ...
  return (
    <div 
      ...
      className="fixed inset-0 bg-slate-950/40 backdrop-blur-md z-50 flex items-center justify-center p-4 transition-all duration-300 animate-fade-in"
    >
      <div 
        id="auth-modal-card" 
        className="relative w-full max-w-md bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-7 shadow-xl overflow-y-auto select-none transition-all duration-300 max-h-[92vh] scrollbar-thin scrollbar-thumb-emerald-500/20"
      >
        
        {/* Minimal Close Button */}
        <button
          onClick={onClose}
          className="absolute top-6 right-6 p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-250 cursor-pointer transition-all duration-250 active:scale-95"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center h-12 w-12 rounded-xl bg-emerald-500/10 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 mb-3.5">
          ...
          <h2 className="text-lg font-bold text-slate-950 dark:text-white tracking-tight">
            {activeTab === "login" && (loginType === "agent" 
              ? "Welcome back, Broker & Client" 
              : "Administrative Portal")}
            {activeTab === "register" && "Create Account"}
            {activeTab === "forgot" && "Reset Passcode"}
          </h2>
          <p className="text-xs text-slate-505 dark:text-slate-400 mt-1.5 leading-relaxed max-w-sm mx-auto">
            {activeTab === "login" && (loginType === "agent" 
              ? "Sign in to manage your listings, view leads, and access real estate tools." 
              : "Administrative space to manage land deed approvals, agencies, and broker vetting.")}
            {activeTab === "register" && "Choose between a Broker or client account to get started."}
            {activeTab === "forgot" && "Enter your email address to receive passcode reset instructions."}
          </p>
        </div>

        {/* Cohesive Sub-Tabs Selection */}
        {activeTab !== "forgot" && (
          <div className="flex bg-slate-100/80 dark:bg-slate-950/60 p-1 rounded-xl mb-5 border border-slate-200/35 dark:border-slate-800/60">
            <button
            ...
            <button
              ...
              className={`flex-1 py-1.5 text-xs font-semibold rounded-lg transition-all cursor-pointer ${
                activeTab === "register"
                  ? "bg-white dark:bg-slate-800 text-slate-955 dark:text-white shadow-sm font-bold"
                  : "text-slate-505 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200"
              }`}
            >
              Sign Up
        ...
        {feedback && (
          <div className={`p-3.5 rounded-xl text-xs flex gap-2.5 items-start mb-5 border animate-fade-in leading-relaxed ${
            ...
            {feedback.type === "success" ? (
              <CheckCircle className="h-4.5 w-4.5 flex-shrink-0 text-emerald-500" />
            ) : (
              <ShieldAlert className="h-4.5 w-4.5 flex-shrink-0 text-rose-500" />
            )}
            <span className="font-medium text-left leading-normal">{feedback.msg}</span>
          </div>
        )}
        ...
        {activeTab === "login" && (
          <div className="space-y-4">
            ...
            {/* Minimal Segmented Switcher for Portal Type */}
            <div className="grid grid-cols-2 p-1 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-850 rounded-xl mb-4">
              <button
                type="button"
                onClick={() => {
                  setLoginType("agent");
                ...
                className={`py-2 px-3 text-xs font-medium rounded-lg flex items-center justify-center gap-2 transition-all cursor-pointer ${
                  loginType === "agent"
              ...
              <button
                ...
                className={`py-2 px-3 text-xs font-medium rounded-lg flex items-center justify-center gap-2 transition-all cursor-pointer ${
                  loginType === "admin"
                    ? "bg-slate-900 text-white dark:bg-slate-850 shadow-sm font-semibold"
                    : "text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200"
                }`}
              >
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4 text-left">
              <div className="space-y-1">
                <label className="text-xs font-medium text-slate-700 dark:text-slate-350 block">
                  {loginType === "admin" ? "Official Admin Email" : "Username or Email Address"}
                </label>
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder={loginType === "admin" ? "admin@domain.com" : "Enter your username or email"}
                    required
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    className="w-full bg-slate-50 dark:bg-slate-950/45 pl-9 pr-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 text-sm outline-none focus:border-emerald-500 dark:focus:border-emerald-500/80 focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-emerald-500/5 transition-all"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-medium text-slate-700 dark:text-slate-355 block">Passcode</label>
                  <button
                    type="button"
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950/45 pl-9 pr-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 text-sm outline-none focus:border-emerald-500 dark:focus:border-emerald-500/80 focus:bg-white dark:focus:bg-slate-905 focus:ring-4 focus:ring-emerald-500/5 transition-all"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                className={`w-full py-2.5 text-white font-semibold text-xs rounded-xl transition-all cursor-pointer shadow-sm hover:shadow-md mt-2.5 uppercase tracking-wider text-center ${
                  loginType === "admin"
                    ? "bg-slate-950 hover:bg-slate-900 dark:bg-slate-800 dark:hover:bg-slate-700"
        ...
        {activeTab === "register" && (
          <div className="space-y-5 py-1 text-left animate-fade-in">
            <div className="p-4 rounded-2xl bg-gradient-to-br from-emerald-500/10 to-teal-500/5 border border-emerald-500/20 text-slate-800 dark:text-emerald-300 text-xs space-y-2">
              <div className="flex gap-2 items-center text-emerald-600 dark:text-emerald-400 font-extrabold text-sm">
                ...
                <span>Welcome to the Broker & Client Registry</span>
              </div>
              <p className="leading-relaxed text-slate-600 dark:text-slate-400">
                Fill in the form below to register your official profile. Registration grants you immediate access to listings and real estate management tools.
              </p>
            </div>
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-4 text-left">
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950/60 pl-10 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 text-sm outline-none focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-900 transition-all font-sans font-medium"
                  />
                </div>
              </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      value={email}
                      onChange={(e) => setEmail(e.target.value)}
                      className="w-full bg-slate-50 dark:bg-slate-950/60 pl-10 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 text-xs outline-none focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-900 transition-all font-sans font-medium"
                    />
                  </div>
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider block">Phone Number</label>
                  <div className="relative group">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider block">Choose Account Role</label>
                <div className="relative group">
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950/60 pl-10 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 text-xs outline-none focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-900 transition-all font-sans font-bold text-slate-800 dark:text-slate-200 cursor-pointer"
                  >
                    <option value="agent">🏢 Broker & Licensed Agent</option>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider block">New Passcode</label>
                <div className="relative group">
              ...
              <button
                type="submit"
                className="w-full py-3.5 bg-gradient-to-r from-emerald-600 to-teal-650 hover:from-emerald-500 hover:to-teal-555 text-white font-black text-xs rounded-xl uppercase tracking-widest transition-all shadow-md shadow-emerald-500/10 cursor-pointer hover:scale-[1.01] active:scale-95 text-center"
              >
                📝 Register Profile
              </button>
            </form>
