...
export default function UserDashboard({
      ...
      if (file.type.startsWith("image/")) {
        ...
        reader.onload = (event) => {
          if (event.target?.result) {
            ...
            setFormImages(prev => {
              const copy = [...prev];
              copy[formImageIndex] = dataUrl;
              return copy;
            });
            setPropFileUploadName(file.name);
          }
        };
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const propertyPayload = {
      ...
      description,
      category,
      type: category === PropertyCategory.LandSale ? "Land" : (category === PropertyCategory.Commercial ? "Commercial" : "Residential"),
      ...
      areaSize: parseFloat(areaSize) || 100,
      images: chosenImages,
      featured: false,
      availableDate: status === PropertyStatus.Rent ? availableDate : undefined
    };
  ...
  return (
    <div id="user-dashboard-root" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 transition-colors">
      ...
      {/* Hello Board Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-gradient-to-r from-slate-900 to-slate-800 dark:from-slate-950 dark:to-slate-900 text-white p-6 sm:p-8 rounded-3xl mb-8 shadow-lg gap-4">
        <div className="flex items-center gap-4">
          <div 
            ...
            className={`relative h-16 w-16 rounded-2xl flex items-center justify-center transition-all ${
              avatarDragActive 
                ? "bg-slate-800 border-2 border-dashed border-emerald-400 scale-105" 
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1 uppercase font-mono tracking-widest text-emerald-400">
        ...
        {/* Dynamic status card */}
        <div className="bg-white/5 border border-white/10 px-5 py-3 rounded-2xl flex gap-6 text-center text-xs">
          <div>
            <p className="text-slate-400 font-medium">{language === "en" ? "My Listings" : "Guryahayga"}</p>
            <p className="text-lg font-bold font-mono text-emerald-400 mt-0.5">{userListings.length}</p>
          </div>
          <div className="border-l border-white/10" />
          <div>
            <p className="text-slate-400 font-medium">{language === "en" ? "Inquiries" : "Fariimaha"}</p>
            <p className="text-lg font-bold font-mono text-emerald-400 mt-0.5">{userInquiries.length}</p>
          </div>
          <div className="border-l border-white/10" />
          <div>
            <p className="text-slate-400 font-medium">{language === "en" ? "Favorites" : "La Jecelyahay"}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3 rounded-xl text-sm font-semibold transition-all cursor-pointer ${
                  active 
                    ...
                    : "bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-350 hover:bg-slate-50 dark:hover:bg-slate-800/60 border border-gray-100 dark:border-slate-850"
                }`}
              >
        ...
        <div className="lg:col-span-3 bg-white dark:bg-slate-905 border border-gray-100 dark:border-slate-850 p-6 sm:p-8 rounded-3xl min-h-[500px] shadow-sm">
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center pb-5 border-b border-gray-100 dark:border-slate-800 gap-4">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs">
                    <span className="bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 px-3 py-1 rounded-full font-bold font-mono">
                      {language === "en" ? "Live Catalogued:" : "Kuwa Idman:"} {liveCount}
                    </span>
                    <span className="bg-slate-100 dark:bg-slate-850 text-slate-600 dark:text-slate-350 px-3 py-1 rounded-full font-bold font-mono">
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center pb-4 border-b border-gray-100 dark:border-slate-800">
                <h3 className="font-display font-bold text-lg text-slate-900 dark:text-white">
                  {editingId ? "Edit Property Specifications" : "Upload Verified Property"}
                </h3>
                <span className="text-[10px] bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded font-bold font-mono">REGISTRY VETTING ACTIVE</span>
              </div>

              <form onSubmit={handleFormSubmit} className="space-y-5 text-left text-xs text-slate-700 dark:text-slate-300">
                    {/* 5-Slot Image Management System */}
                <div className="space-y-3 bg-slate-50 dark:bg-slate-900/45 p-4 rounded-2xl border border-gray-150 dark:border-slate-800">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pb-2 border-b border-gray-100 dark:border-slate-800">
                    <div>
                      <h4 className="font-bold text-slate-800 dark:text-slate-200 text-xs sm:text-sm uppercase tracking-wide">
                        {language === "en" ? "📸 Image Gallery (Max 5 Slots)" : "📸 Maamulka Sawirrada (Ugu badnaan 5 Sawir)"}
                      </h4>
                      <p className="text-[10px] text-slate-400 mt-0.5">
                        {language === "en" 
                          ? "Select a slot below, then upload/paste a link or choose a preset option." 
                          : "Giriidka hoose ka dooro booska, dabadeed soo geli ama dooro sawir."}
                      </p>
                    </div>
                    <span className="text-[10px] bg-emerald-500/10 text-emerald-500 px-2 py-1 rounded font-mono font-bold uppercase">
                      {language === "en" ? `Active Slot: #${formImageIndex + 1}` : `Kala dooro: Booska #${formImageIndex + 1}`}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-5 gap-2 sm:gap-3">
                    {formImages.map((img, idx) => {
                      const isActive = formImageIndex === idx;
                      const hasImage = img && img.trim() !== "";
                      return (
                        <div
                          key={idx}
                          id={`form-image-slot-${idx}`}
                          onClick={() => setFormImageIndex(idx)}
                          className={`relative aspect-[4/3] rounded-xl overflow-hidden border-2 cursor-pointer transition-all duration-250 flex flex-col items-center justify-center p-1 ${
                            isActive
                              ? "border-emerald-500 bg-emerald-50/10 dark:bg-emerald-950/20 ring-2 ring-emerald-500/15"
                              : hasImage
                              ? "border-transparent bg-slate-150 dark:bg-slate-800 hover:border-gray-300 dark:hover:border-slate-700"
                              : "border-dashed border-gray-200 dark:border-slate-850 hover:border-emerald-500/20"
                          }`}
                        >
                          {hasImage ? (
                            <>
                              <img src={img} className="w-full h-full object-cover rounded-lg" alt={`Slot ${idx + 1}`} />
                              <button
                                type="button"
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                className="absolute top-1 right-1 h-5 w-5 bg-black/75 hover:bg-red-600 text-white flex items-center justify-center rounded-full transition-colors cursor-pointer"
                            ...
                            <div className="flex flex-col items-center justify-center text-slate-400 dark:text-slate-650">
                              ...
                              <span className="text-[8px] sm:text-[9px] font-mono mt-0.5 font-bold">#{idx + 1}</span>
                            </div>
                          )}

                          {/* Cover Image badge for Slot 0 */}
                          {idx === 0 && (
                            <span className="absolute bottom-1 left-1 bg-emerald-600 text-white text-[7px] sm:text-[8px] px-1 rounded font-bold uppercase tracking-widest scale-90 origin-left">
                ...
                {/* Preset Picker applied to active spot */}
                <div className="space-y-2">
                  <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">
                    {language === "en" ? `Choose Preset Image for Slot #${formImageIndex + 1}` : `Ku dar Booska #${formImageIndex + 1} mid ka mid ah Sawirada diyaarka ah`}
                  </label>
                    ...
                    {PRESET_MOCK_IMAGES.map((img, i) => (
                      <button
                        ...
                        onClick={() => {
                          setFormImages(prev => {
                            const copy = [...prev];
                            copy[formImageIndex] = img;
                            return copy;
                          });
                        }}
                        className={`relative aspect-[4/3] rounded-lg overflow-hidden border-2 transition-all ${
                          formImages[formImageIndex] === img 
                      ...
                      >
                        ...
                        <span className="absolute bottom-1 right-1 bg-slate-950/80 text-[8px] px-1 rounded text-white font-mono">#{i+1}</span>
                      </button>
                    ))}
                  </div>
                </div>

                {/* Upload or URL sections applied to active spot */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  
                  {/* File Upload card */}
                  <div className="space-y-2">
                    <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">
                      {language === "en" ? `Upload custom file for Slot #${formImageIndex + 1}` : `Soo geli sawirka Booska #${formImageIndex + 1}`}
                    </label>
                    <div
                      ...
                      className={`border-2 border-dashed rounded-2xl p-5 transition-all text-center flex flex-col items-center justify-center cursor-pointer min-h-[140px] ${
                        propDragActive 
                          ? "border-emerald-500 bg-emerald-50/20 dark:bg-emerald-950/20" 
                          : "border-gray-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 hover:border-emerald-500/40"
                      }`}
                    ...
                    >
                      ...
                      {formImages[formImageIndex] && formImages[formImageIndex].startsWith("data:image") ? (
                        <div className="space-y-1">
                          <img 
                            src={formImages[formImageIndex]} 
                            alt="Uploaded Preview" 
                            className="mx-auto h-16 w-24 object-cover rounded-xl shadow-md border border-gray-150" 
                          />
                          <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold">
                            ✓ {propFileUploadName || "Image.jpg"}
                          </p>
                          <p className="text-[9px] text-slate-450 font-mono">
                            {language === "en" ? "Drag replacement here" : "Bedel u soo jiid halkan"}
                          </p>
                        </div>
                      ) : (
                        <div className="space-y-1.5">
                          <div className="h-8 w-8 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mx-auto">
                            <Upload className="h-4 w-4" />
                          </div>
                          <p className="text-[10px] font-bold text-slate-700 dark:text-slate-350">
                            {language === "en" ? "Drag file here or click to browse" : "Soo jiid sawirka ama guji si aad u dooratid"}
                          </p>
                  ...
                  <div className="space-y-2 flex flex-col justify-between">
                    <div className="space-y-1.5">
                      <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">
                        {language === "en" ? `Or provide custom URL for Slot #${formImageIndex + 1}` : `Amase geli sawirka Link-giisa Booska #${formImageIndex + 1}`}
                      </label>
                      <div className="relative">
                        <Upload className="absolute left-3 top-3 h-4 w-4 text-slate-400" />
                        <input
                          type="url"
                          placeholder="https://images.unsplash.com/your-custom-luxury-mansion..."
                          value={formImages[formImageIndex] && !formImages[formImageIndex].startsWith("data:") ? formImages[formImageIndex] : ""}
                          onChange={(e) => {
                            const url = e.target.value;
                            setFormImages(prev => {
                              const copy = [...prev];
                              copy[formImageIndex] = url;
                              return copy;
                            });
                            setPropFileUploadName("");
                          }}
                          className="w-full bg-slate-50 dark:bg-slate-950 pl-10 pr-4 py-2.5 rounded-xl border border-gray-100 dark:border-slate-800 text-xs font-sans text-slate-800 dark:text-slate-200 outline-none focus:border-emerald-500"
                        />
                      </div>
                    </div>

                    <div className="bg-emerald-500/5 p-3 rounded-xl border border-emerald-500/10 text-[10px] text-emerald-600 dark:text-emerald-400/90 leading-relaxed font-sans mt-auto">
                      💡 <strong>{language === "en" ? "Pro Tip: " : "Ogow: "}</strong>
                      {language === "en"
                        ? "The first image of the 5 slots will act as the principal 'Cover Photo' throughout the Abudwak catalog viewports!"
                        : "Sawirka ugu horeeya ee booska #1 wuxuu u adeegi doonaa sawirka ugu horeeya (Cover Photo) ee ku muuqan doona liiska!"}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest block">Property Title</label>
                    <input
                      type="text"
                      placeholder="e.g. Modern Oceanfront duplex luxury home"
                      required
                      value={title}
                      onChange={(e) => setTitle(e.target.value)}
                      className="w-full bg-slate-50 dark:bg-slate-950 px-4 py-2.5 rounded-xl border border-gray-250 dark:border-slate-800/80 font-medium text-xs text-slate-800 dark:text-slate-200 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/10 hover:border-slate-300 dark:hover:border-slate-700 transition-all font-sans"
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest block">Listed Price (USD)</label>
                    <input
                      type="number"
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-950 px-4 py-2.5 rounded-xl border border-gray-250 dark:border-slate-800/80 font-mono text-xs text-slate-800 dark:text-slate-200 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/10 hover:border-slate-300 dark:hover:border-slate-700 transition-all"
                    />
                  </div>
                </div>

                {/* Region, specific street address, and property status */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest block">Xaafadda / Neighborhood</label>
                    <select
                      value={region}
                      onChange={(e) => setRegion(e.target.value)}
                      className="w-full bg-slate-50 dark:bg-slate-950 px-4 py-2.5 rounded-xl border border-gray-250 dark:border-slate-800/80 text-xs text-slate-800 dark:text-slate-200 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/10 hover:border-slate-300 dark:hover:border-slate-700 transition-all cursor-pointer font-sans"
                    >
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest block">Street Address / Zone Location</label>
                    <input
                      type="text"
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-950 px-4 py-2.5 rounded-xl border border-gray-255 dark:border-slate-800/80 text-xs text-slate-800 dark:text-slate-200 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/10 hover:border-slate-300 dark:hover:border-slate-700 transition-all font-sans"
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest block">Transaction Status</label>
                    <select
                      value={status}
                      onChange={(e) => setStatus(e.target.value as PropertyStatus)}
                      className="w-full bg-slate-50 dark:bg-slate-950 px-4 py-2.5 rounded-xl border border-gray-255 dark:border-slate-800/80 text-xs text-slate-800 dark:text-slate-200 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/10 hover:border-slate-300 dark:hover:border-slate-700 transition-all cursor-pointer font-sans"
                    >
                      <option value={PropertyStatus.Sale}>For Sale</option>
                      <option value={PropertyStatus.Rent}>For Rent</option>
                    </select>
                  </div>
                </div>

                {/* Rent Availability Date (Soo galinta rentiga raaci date) */}
                {status === PropertyStatus.Rent && (
                  <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 space-y-2">
                    <label className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider block">
                      Taariikhda la dhex-gelayo/la bilaabayo kirada (Rent Availability & Start Date)
                    </label>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500">
                      Dooro xilliga guriga la kireynayo uu diyaar yahay dhex-galkiisa (Please specify the date this rental is ready for lease).
                    </p>
                    <input
                      type="date"
                      value={availableDate}
                      onChange={(e) => setAvailableDate(e.target.value)}
                      required={status === PropertyStatus.Rent}
                      className="max-w-xs w-full bg-slate-50 dark:bg-slate-950 px-4 py-3 rounded-xl border border-gray-100 dark:border-slate-800 font-mono text-xs text-slate-800 dark:text-slate-200 outline-none focus:border-emerald-500"
                    />
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest block">Category type</label>
                    <select
                      value={category}
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-950 px-4 py-2.5 rounded-xl border border-gray-255 dark:border-slate-800/80 text-xs text-slate-850 dark:text-slate-150 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/10 hover:border-slate-300 dark:hover:border-slate-700 transition-all"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-1">
                      <Maximize className="h-3 w-3" /> Area Size (Sq M)
                    </label>
                    <input
                      type="number"
                      placeholder="e.g. 280"
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest block mb-1">Detailed Property Description & Key features</label>
                  <textarea
                    rows={4}
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-gray-255 dark:border-slate-800/80 resize-none font-sans text-xs text-slate-800 dark:text-slate-200 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/10 hover:border-slate-300 dark:hover:border-slate-700 transition-all"
                  />
                </div>
